...
from moltrans.data import BMSExtraMolDataset, BMSSmilesDataModule
from moltrans.model import BARTModel
...
def build_trainer(args):
    ...
    logger = TensorBoardLogger(LOG_DIR, name="bms-decoder")
    lr_monitor = LearningRateMonitor(logging_interval="step")
    checkpoint_cb = ModelCheckpoint(monitor="val_loss", save_last=True)
...
def main(args):
    ...
    dataset = BMSExtraMolDataset.from_data_path(args.data_path)
    ...
    dm = BMSSmilesDataModule(train_dataset, val_dataset, args.batch_size, tokeniser)
